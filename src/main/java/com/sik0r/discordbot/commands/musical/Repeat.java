...
public class Repeat implements ICommand {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        boolean repeat = !playerManager.getGuildMusicManager(event.getGuild()).getTrackScheduler().isRepeat();
        playerManager.getGuildMusicManager(event.getGuild()).getTrackScheduler().setRepeat(repeat);
        builder.setTitle("Loop");
        builder.setDescription(repeat ? "Трек залуплен" : "Трек разлуплен");
        event.replyEmbeds(builder.build()).queue();
    }
}
